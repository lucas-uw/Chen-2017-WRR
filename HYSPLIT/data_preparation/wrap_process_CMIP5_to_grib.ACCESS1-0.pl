...
for($month=1; $month<=12; $month++) {
  ...
  $cmd = "$toolsdir/clip_CMIP5_data.pl $model $year $month $rootdir";
  (system($cmd)==0) or die "$0: ERROR: $cmd failed\n";

  # assemble into one big file
  $cmd = "$toolsdir/nc2bignc.ACCESS1-0.py $model $year $month $rootdir/$model/remap_by_month day.$model $rootdir/$model/monthly_ext";
  (system($cmd)==0) or die "$0: ERROR: $cmd failed\n";
  ...
  $cmd = "cd /usr1/xiaodong.chen/lulcc/models/hysplit/xc;grib2arl -i$model.$year.$month.12hr.ext.grib1 -cECMWF_ref/ACCESS_orog.grib";
  print "$cmd\n";
  (system($cmd)==0) or die "$0: ERROR: $cmd failed\n";

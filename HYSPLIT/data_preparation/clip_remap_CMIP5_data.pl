...
$out_dir = "$rootdir/$model/clip_by_month";
$remap_dir = "$rootdir/$model/remap_by_month";
...
foreach $var(@varlist) {
	...
	$cmd = "cdo remapbil,$ECMWF_gridfile $tmpfile_month $tmpfile_remap";
	print "# remapping\n$cmd\n";
	(system($cmd)==0) or die "$0: ERROR: $cmd failed\n";

	$cmd = "rm $tmpfile_month";
	print "# cleaning\n$cmd\n";
	(system($cmd)==0) or die "$0: ERROR: $cmd failed\n";
}

...
for($month=1; $month<=12; $month++) {
  ...
  $cmd = "$toolsdir/clip_CMIP5_data.red_var.pl $model $year $month $rootdir";
  ...
  $cmd = "cd /usr1/xiaodong.chen/lulcc/models/hysplit/xc;grib2arl -i$model.$year.$month.12hr.ext.grib1 -cECMWF_ref/GFDL-ESM2G_orog.grib";
  print "$cmd\n";
  (system($cmd)==0) or die "$0: ERROR: $cmd failed\n";

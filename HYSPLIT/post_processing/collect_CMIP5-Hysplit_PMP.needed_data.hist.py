...
def collect_storm_rep_sst1(huidstr, ne):
    ...
    for i in np.arange(hysplit_results.shape[0]):
        if hysplit_results[i,6]<200 and hysplit_results[i,7]>0:
            ratio_tmp = sst2pw(hysplit_results[i,8]-273.12)/sst2pw(hysplit_results[i,7]-273.12) 
            if ratio_tmp>ratio:
                ratio = ratio_tmp
...
for n in np.arange(220):
    ...
    infile = inputdir + '/top200/'+model+'/'+period+'/'+huidstr+'.'+model+'.'+period+'.txt'
    outfile = outputdir + '/results_for_RT_experiments/10day_results/'+model+'/'+period+'/H'+Hvalue+'/'+huidstr+'.'+model+'.H'+Hvalue+'.'+period
